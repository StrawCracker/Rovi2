...
bool caros::TrakstarNode::activateHook()
{
  ...
  nodehandle_.param("frame", frame_id_, std::string("trakstar_base"));
...
void caros::TrakstarNode::runLoopHook()
{
  ...
  {
    // The rotation outputs of the trakstar are measured in transmitter
    // frame wrt. sensor frame. The rotational part is inverted as we wishde
    // to publish the rotation of the sensor given wrt. the transmitter frame.
    ids.push_back(i);
    rw::math::Rotation3D<> rot = pd[i].rot.toRotation3D();
    transforms.push_back(rw::math::Transform3D<>(pd[i].pos * 0.001, rot.inverse()));
    qualities.push_back(pd[i].quality);
  }
  if (pd[0].analog_button_on)
  {
  ...
  PoseSensorServiceInterface::publishPoses(transforms, ids, qualities, frame_id_);
