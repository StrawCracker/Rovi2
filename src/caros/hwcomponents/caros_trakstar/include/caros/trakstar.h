...
class Trakstar
{
 ...
 public:
  ...
  const std::vector<PoseData>& getData();
 ...
 private:
  tagSYSTEM_CONFIGURATION* ATC3DG_;  // a pointer to a single instance of the system class
  // a pointer to an array of sensor objects
  std::vector<rw::common::Ptr<tagSENSOR_CONFIGURATION>> sensors_;
  // a pointer to an array of transmitter objects
  std::vector<rw::common::Ptr<tagTRANSMITTER_CONFIGURATION>> transmitters_;
  std::vector<TRAKSTAR_RECORDS_TYPE> raw_values_;

  std::vector<PoseData> records_;
