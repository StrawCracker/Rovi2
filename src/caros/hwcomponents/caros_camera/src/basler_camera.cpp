...
bool BaslerCamera::getRawImage(sensor_msgs::ImagePtr& img, uint64_t& timestamp)
{
  ...
  if (ptr_grab_result->GrabSucceeded())
  {
    ...
    uchar copy[rows * cols];  // NOLINT(runtime/arrays)
                                   ...
                                   std::map<std::string, double> double_parameters)
{
  ...
  if (gain_raw.IsValid())
  {
    int64_t new_gain_raw =
        gain_raw->GetMin() + int_parameters["GainRaw"];  // ((gain_raw->GetMax() - gain_raw->GetMin()) / 2)
    ...
    std::cout << "Gain       : " << gain_raw->GetValue() << " (Min: " << gain_raw->GetMin()
              << "; Max: " << gain_raw->GetMax() << "; Inc: " << gain_raw->GetInc() << ")" << std::endl;
  }
   ...
   */
  setParam<GenApi::IInteger>(nodemap, "GevSCPSPacketSize", parameters["GevSCPSPacketSize"]);  // MTU in bytes
