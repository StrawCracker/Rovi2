...
const Services_t services_to_test = {
    {std::make_tuple(
        ...
        "virtual bool SerialDeviceServiceInterfaceDummy::moveLin(const TransformAndSpeedAndBlendContainer_t&)")},
    {std::make_tuple(std::bind(&caros::SerialDeviceSIProxy::movePtp, std::placeholders::_1, rw::math::Q(), 0, 0),
                     "virtual bool SerialDeviceServiceInterfaceDummy::movePtp(const QAndSpeedAndBlendContainer_t&)")},
    {std::make_tuple(
        std::bind(&caros::SerialDeviceSIProxy::movePtpT, std::placeholders::_1, rw::math::Transform3D<>(), 0, 0),
        "virtual bool SerialDeviceServiceInterfaceDummy::movePtpT(const TransformAndSpeedAndBlendContainer_t&)")},
    {std::make_tuple(
